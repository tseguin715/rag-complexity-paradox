...
# 4. DRAW THE TIGHTER BOX AROUND LLAMA FAILURES
# ---------------------------------------------------------
llama_fail_points = df[
    (df['llm'].str.contains('iq2', case=False)) & 
    (df['Article_System'] == 'System D (God Mode)')
]

if not llama_fail_points.empty:
    x_min, x_max = llama_fail_points['main_total_tokens'].min(), llama_fail_points['main_total_tokens'].max()
    y_min, y_max = llama_fail_points['correctness_score'].min(), llama_fail_points['correctness_score'].max()
    
    # ADJUST PADDING HERE
    pad_x = 250   # Horizontal padding
    pad_y = 0.05  # REDUCED Vertical padding (was 0.08)
    ...
    fig.add_annotation(
        x=(x0 + x1) / 2, 
        y=(y0 + y1) / 2, 
        text="<b>System D with Llama:</b><br>High cost, low accuracy",
        showarrow=False,
        font=dict(size=12, color="#7d3c98"),
...
::: {.fig-caption}
**Figure 9**. 95% bootstrap confidence intervals for mean correctness differences between pairs of RAG architectures. The difference in correctness is statistically insignificant between System C and Systems A and B, but statistically significant between Systems C and D.  
:::
...
llama_4bit = "evals/evaluation_results_llama3.1_70b_SysG_v4_Reranked_FIXED_1767519618.jsonl"
llama_2bit = "evals/evaluation_results_llama3.1-70b-iq2_xs_SysG_v4_Reranked_FIXED_1767510960.jsonl"
...
In LLMs, there is an "attention mechanism" that relies on precise floating point numbers to relate words to one another. These relationships degrade with each quantization step (e.g., FP16 --> FP 8 --> INT8 --> INT4 --> ...) until the LLM loses the ability to distinguish which data (document) is correct for retrieval or how to follow complex instructions. The degradation isn't uniform: sometimes, the reasoning or syntax capabilities are the most affected.

The fact that Qwen 2.5 32B didn't appear to suffer a severe degradation from 4-bit to 2-bit is remarkable. It must have been well-tuned against the expected performance degradation from such aggressive quantization. The same appears less true for Llama 3.1 70B, but because the 4-bit model also showed somewhat lackluster performance, it appears as though its architecture -- its ability to reason, distinguish signal from noise, and follow instructions -- wasn't as well suited for the highly complex System D in the first place.
...
To summarize, these findings suggest the existence of a fragility threshold in certain large models: beyond a particular level of system complexity, additional control mechanisms may no longer compensate for noise, and may instead exacerbate it. This has practical implications for those seeking to deploy highly agentic RAG systems on constrained hardware. In such cases, it's important to keep in mind that smaller or more quantization-tolerant models may outperform larger models.

# Conclusions

These would be my recommended configurations for production-level deployment:
...
- **What to Avoid:**
  ...
  - 2-bit quantization with complex architectures (unless thoroughly validated for robustness)
...
A lackluster performance by 2-bit models like Llama 3.1 70B-IQ2 is perhaps unsurprising. However, when investigating the effects of moving from 4-bit to 2-bit, it was shown that the degradation for Llama 3.1 70B was markedly worse than for Qwen 2.5 32B. Therefore, performance appears to be shaped by a complex interplay between model architecture, quantization, and an error-amplifying effect of the complex System D pipeline.

It should be noted that the most aggressively quantized model -- Llama 3.1 70B-IQ2 -- is a community-produced release, rather than an official Meta release. Therefore, these results should not be interpreted as definitive judgments on the Llama architecture itself. Moreover, newer open-source models (e.g., Llama 3.3) were released after the completion of this study and not evaluated here. 
